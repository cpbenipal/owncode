...
using PanelMasterMVC5Separate.Tests.TestDatas;
...
namespace PanelMasterMVC5Separate.Tests
{
    ...
    /// Provides methods to easily work with <see cref="PanelMasterMVC5SeparateDbContext"/>.
    /// </summary>
    public abstract class AppTestBase : AbpIntegratedTestBase<PanelMasterMVC5SeparateTestModule>
    {
        ...
        protected void UsingDbContext(Action<PanelMasterMVC5SeparateDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<PanelMasterMVC5SeparateDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<PanelMasterMVC5SeparateDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<PanelMasterMVC5SeparateDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<PanelMasterMVC5SeparateDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<PanelMasterMVC5SeparateDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<PanelMasterMVC5SeparateDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<PanelMasterMVC5SeparateDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<PanelMasterMVC5SeparateDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<PanelMasterMVC5SeparateDbContext, Task<T>> func)
        {
            T result;

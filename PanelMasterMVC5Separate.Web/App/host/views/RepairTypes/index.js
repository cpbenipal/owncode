(function () {
    ...
    appModule.controller('host.views.RepairTypes.index', [
        ...
        function ($scope, $uibModal, $stateParams, uiGridConstants, userService) {
            ...
            vm.getRepairType = function () {
                vm.loading = true;
                userService.getRepairTypes($.extend({ filter: vm.filterText }, vm.requestParams))
            ...
            vm.exportToExcel = function () {
                userService.getRepairTypeToExcel({})
                    .then(function (result) {
            ...
            function openCreateOrEditUserModal(quoteId) {
                ...
                var modalInstance = $uibModal.open({
                    templateUrl: '~/App/host/views/RepairTypes/createOrEditModal.cshtml',
                    controller: 'host.views.RepairTypes.createOrEditModal as vm',
                    backdrop: 'static',
                    resolve: {
                        quoteId: function () {
                            return quoteId;
            ...
            vm.Status = function (mask) {
                abp.message.confirm(
                    ...
                    function (isConfirmed) {
                        if (isConfirmed) {
                            if (mask.id != 0) {
                                userService.changeRepairTypeStatus({
                                    id: mask.id,
                                    status: !mask.enabled
                                }).then(function () {
                                    vm.getRepairType();

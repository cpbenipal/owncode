(function () {
    ...
    appModule.controller('host.views.VehicleMades.index', [
        ...
        function ($scope, $uibModal, $stateParams, uiGridConstants, userService) {
                ...
                columnDefs: [
                    {
                        ...
                        '      <li><a ng-if="grid.appScope.permissions.edit" ng-click="grid.appScope.edit(row.entity.madeID)">' + app.localize('Open') + '</a></li>' +                         
                        '    </ul>' +
                        '  </div>' +
                        '</div>'
                    },
            ...
            vm.edit = function (madeId) {
                openCreateOrEditUserModal(madeId);
            };

            vm.newBroker = function () {
                openCreateOrEditUserModal(null);
            };
            function openCreateOrEditUserModal(madeId) {

                var modalInstance = $uibModal.open({
                    templateUrl: '~/App/host/views/VehicleMades/createOrEditModal.cshtml',
                    controller: 'host.views.VehicleMades.createOrEditModal as vm',
                    backdrop: 'static',
                    resolve: {
                        madeId: function () {
                            return madeId;

(function () {
    ...
    appModule.controller('host.views.QuoteStatus.index', [
        ...
        function ($scope, $uibModal, $stateParams, uiGridConstants, userService) {
            ...
            vm.getQuoteStatus = function () {
                vm.loading = true;
                userService.getQuoteStatuses($.extend({ filter: vm.filterText }, vm.requestParams))
            ...
            vm.exportToExcel = function () {
                userService.getQuoteStatusToExcel({})
                    .then(function (result) {
            ...
            function openCreateOrEditUserModal(quoteId) {
                ...
                var modalInstance = $uibModal.open({
                    templateUrl: '~/App/host/views/QuoteStatus/createOrEditModal.cshtml',
                    controller: 'host.views.QuoteStatus.createOrEditModal as vm',
                    backdrop: 'static',
                    resolve: {
                        quoteId: function () {
                            return quoteId;
            ...
            vm.Status = function (mask) {
                abp.message.confirm(
                    ...
                    function (isConfirmed) {
                        if (isConfirmed) {
                            if (mask.id != 0) {
                                userService.changeQuoteStatusStatus({
                                    id: mask.id,
                                    status: !mask.enabled
                                }).then(function () {
                                    vm.getQuoteStatus();

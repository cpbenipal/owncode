(function () {
    ...
    appModule.controller('tenant.views.jobstatuses.index', [
        ...
        function ($scope, $uibModal, $stateParams, uiGridConstants, userService) {
                ...
                columnDefs: [
                    {
                        ...
                        '      <li><a ng-show="row.entity.isActive" ng-if="grid.appScope.permissions.edit" ng-click="grid.appScope.editJobStatus(row.entity.id)">' + app.localize('Edit') + '</a></li>' +
                        '    </ul>' +
                        '  </div>' +
                        '</div>'
                    },
                    ...
                    {
                        name: app.localize('ShowSpeedbump'),
                        field: 'showSpeedbump',
                        minWidth: 120
                    },
                    {
                        name: app.localize('ShowAwaiting'),
                        field: 'showAwaiting',
                        minWidth: 120
                    },
                    {
                        name: app.localize('JobstatusMask'),
                        field: 'jobstatusMask',
                        minWidth: 120
                    },
                    {
            ...
            function openCreateOrEditUserModal(jobStatusId) {
                ...
                var modalInstance = $uibModal.open({
                    templateUrl: '~/App/tenant/views/jobstatuses/createOrEditModal.cshtml',
                    controller: 'tenant.views.jobstatuses.createOrEditModal as vm',
                    backdrop: 'static',
                    resolve: {
            ...
            vm.Status = function (quote) {
                abp.message.confirm(
                    app.localize('AreYouSure', quote.jobStatus),
                    function (isConfirmed) {
                        if (isConfirmed) {
                            if (quote.pkId != 0) {
                                ...
                                openCreateOrEditUserModal(quote.id);
                            }
                        }
                    }
                );

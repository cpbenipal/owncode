...
CellEditor.prototype.applyEditing = function (element, newValue) {
    with (this) {
        ...
        if (element && element.isEditing) {
            ...
            var qty = editablegrid.getValueAt(element.rowIndex, 6);
            var price = editablegrid.getValueAt(element.rowIndex, 7);
            var vprice = editablegrid.setValueAt(element.rowIndex, 8, qty * price);

            var vqvalue = editablegrid.getValueAt(element.rowIndex, 8);
           ...
           // var setpbrate = editablegrid.setValueAt(element.rowIndex, 11, pbvalue / pbhrs);
            ...
            var prate = editablegrid.getValueAt(element.rowIndex, 14);
            var setpvalue = editablegrid.setValueAt(element.rowIndex, 15, phrs * prate);

            var sahrs = editablegrid.getValueAt(element.rowIndex, 16);
            var sarate = editablegrid.getValueAt(element.rowIndex, 17);
            var setasvalue = editablegrid.setValueAt(element.rowIndex, 18, sahrs * sarate);
